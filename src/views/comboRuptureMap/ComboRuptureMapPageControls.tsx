...
const ComboRuptureMapControls: React.FC<ComboRuptureMapControlsProps> = ({
  ...
  const data = useLazyLoadQuery<ComboRuptureMapPageControlsQuery>(comboRuptureMapPageControlsQuery, {
  ...
  const handleClickSurfaceDownload = () =>
    handleDownloadData(faultSurfacesGeojson, "fault-surfaces.geojson", "geoJSON");
  ...
  return (
    <Box sx={{ width: "100%", ...flexParentCenter, flexDirection: "column" }}>
      ...
      <Box sx={{ ...flexParentCenter, flexDirection: "row" }}>
        <StyledButton
          disabled={isPending || !!radiusError || dataFetched}
          variant="contained"
          type="submit"
      ...
      <Box sx={{ width: "100%", ...flexParentCenter, flexDirection: "row", marginRight: "10px" }}>
        <MapViewControls
          initState={{ showSurfaces: true, showAnimation: true, showMfd: true, showTraceLegend: true }}
          onHandleChange={mapViewControlsDispatch}
        />
